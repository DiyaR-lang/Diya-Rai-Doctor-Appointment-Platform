...
export default function DoctorDashboard() {
  ...
  // Filter unique patients from confirmed appointments for the chat list
  ...
  useEffect(() => {
    ...
    // 1. Initialize Socket with stable transport settings
  ...
  function ChatBox({ doctorId, patient, mainSocket }) {
    ...
    useEffect(() => {
      ...
      // Join the specific chat room on the backend
      ...
      <div className="flex flex-col h-full bg-white border rounded-3xl overflow-hidden shadow-sm">
        <div className="p-5 bg-slate-900 text-white flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={initiateVideoCall} className="text-[10px] font-black bg-blue-600 px-4 py-2 rounded-xl hover:bg-blue-700 transition-all">
              📞 VIDEO CALL
            </button>
                ...
                <div className={`max-w-[75%] p-4 rounded-2xl ${msg.senderId === doctorId ? "bg-green-600 text-white" : "bg-white border text-slate-800"}`}>
                  {isVideoCall ? (
                    <div className="text-center p-2">
                      <p className="text-[9px] font-black uppercase opacity-70 mb-2">Video Session</p>
                    ...
                    <img src={msg.message} className="rounded-lg max-h-60" alt="Attachment" />
                  ) : (
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        <aside className="lg:col-span-1 space-y-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 text-center shadow-sm">
            {/* Added null check to prevent empty src error */}
          ...
          <nav className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            {[
              ...
              <button key={t.id} onClick={() => setActiveTab(t.id)} className={`w-full flex items-center justify-between px-6 py-4 text-sm font-bold transition-all ${activeTab === t.id ? "bg-green-600 text-white" : "text-slate-500 hover:bg-slate-50"}`}>
                ...
                {t.badge > 0 && <span className="bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full">{t.badge}</span>}
              </button>
            ))}
          </nav>
        </aside>
        ...
        <main className="lg:col-span-3 bg-white border border-slate-200 rounded-3xl p-6 md:p-10 shadow-sm min-h-[700px]">
                ...
                <div key={a._id} className="p-5 border border-slate-100 rounded-2xl flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    {a.status === "pending" ? (
                      ...
                      <span className="text-[10px] font-black uppercase px-4 py-1 rounded-full bg-green-100 text-green-600">{a.status}</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          {activeTab === "notifications" && (
            <div className="space-y-4">
              ...
              {notifications.map((n) => (
                <div key={n._id} onClick={() => !n.isRead && markAsRead(n._id)} className={`p-5 rounded-2xl border ${n.isRead ? "bg-white opacity-60" : "bg-green-50 border-green-100"}`}>
